...
- 实现全部 35 条指令：从寄存器操作类（6xkk, 7xkk, 8xy0〜8xyE）开始，逐步覆盖控制流・描画・输入・定时器类
- 用 `test/test_cpu.c` 做指令单元测试
